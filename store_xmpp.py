...
def datastore_daemon():
   ...
   while(True):
      ...
      for key in import_json_temp:
         ...
         commands.getoutput(bin_dir + 'import ' + store + ' 1 ' 
...
def subscribe_daemon(xmpp):
    while(True):
       
        affils = []
        d = xmpp['xep_0060'].get_affiliations(xmpp.pubsub, None)
        for aff in d['pubsub']['affiliations']:
            affils.append(aff['node'])

        subs = []
        d = xmpp['xep_0060'].get_subscriptions(xmpp.pubsub, None)
        for sub in d['pubsub']['subscriptions']:
            subs.append(sub['node'])
        
        for aff in affils:
            if((aff not in subs) and ('_favorites' not in aff)):
                xmpp['xep_0060'].subscribe(xmpp.pubsub, aff)

        for sub in subs:
            if((sub not in affils) and ('_favorites' not in sub)):
                xmpp['xep_0060'].unsubscribe(xmpp.pubsub, sub)
...
class PubsubEvents(sleekxmpp.ClientXMPP):
    ...
    def start(self, event):
        ...
        #thr2 = threading.Thread(target=subscribe_daemon, args=(self,))
        #thr2.daemon = True
        #thr2.start()
